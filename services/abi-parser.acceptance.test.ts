...
import { CallDataInformation, parseCallData } from './parser'
...
const validateAndExtractSwapData = (info: CallDataInformation): Array<LibSwap.SwapDataStruct> => {
    expect(info.functionParameters._swapData).toBeDefined()
    return info.functionParameters._swapData as Array<LibSwap.SwapDataStruct>
}
...
describe('Acceptance tests', () => {
    ...
    it('should parse a swap transfer', async () => {
        ...
        const swapDataCalls = (await Promise.all(swapData.map((s) => s.callData))).flat()
    ...
    it('bridge + swap transfer', async () => {
        ...
        // @ts-ignore
        const [toolCallData] = (await amarokData?.callData) as CallDataInformation[]
        ...
        expect(toolCallData.functionParameters).toEqual(parsed)
    })

    it('swap + stargate + swap transfer', async () => {
        ...
        // @ts-ignore
        const [toolCallData] = (await stargateData).callData as CallDataInformation[]
        expect(toolCallData).toBeDefined()
        const rawCallData =
