...
export const getAbi = async (chainConfig: ChainConfig, address: string): Promise<string | undefined> => {
    const res = await superagent
        ...
        .retry(5, (err, res) => {
            return res.status === 200 && res.body.result.startsWith('Max rate limit reached')
        })
    ...
    if (res.body.message.startsWith('OK')) {
        ...
        console.log(red(`(${res.statusCode}) could not load abi for ${address} on ${chainConfig.chain}: ${res.body.message}`))
    }
